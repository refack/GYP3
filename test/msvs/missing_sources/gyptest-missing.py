...
if sys.platform == 'win32':
  ...
  test.must_contain_any_line(test.stdout(), ["Missing input files:"])

  # With the flag explicitly set to 0
  with TestGyp.LocalEnv({'GYP_GENERATOR_FLAGS': 'msvs_error_on_missing_sources=0'}):
    test.run_gyp('hello_missing.gyp')
    test.must_contain_any_line(test.stdout(), ["Missing input files:"])

  # With the flag explicitly set to 1
  with TestGyp.LocalEnv({'GYP_GENERATOR_FLAGS': 'msvs_error_on_missing_sources=1'}):
    ...
    test.must_contain_any_line(test.stderr(), ["Missing input files:"])
