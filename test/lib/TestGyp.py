...
def FindVisualStudioInstallation():
  ...
  args = ['powershell', '-NoProfile', '-ExecutionPolicy', 'Unrestricted',
          '%s IsVcCompatible MSBuildPath' % tool_path]
  try:
      import subprocess
      msbuild_path = subprocess.check_output(args).split('\n')[0].strip()
  ...
  possible_roots = ['%s:\\Program Files%s' % (chr(drive), suffix)
                    ...
                    for suffix in ['', ' (x86)']]
  possible_paths = {
      '2017': r'Microsoft Visual Studio 15.0\Common7\IDE\devenv.com',
  ...
  if msvs_version in ['2017', 'auto']:
    try:
      import subprocess
      cmdPath = os.path.join(os.path.dirname(__file__), '..', '..',
                             'tools', 'vc141helper', 'get_key_helper.cmd')
      args = [cmdPath, 'IsVcCompatible', 'MSBuildPath']
      msbuild_Path = subprocess.check_output(args).strip()
      if os.path.exists(msbuild_Path):
        return msbuild_Path, True, msbuild_Path
    except Exception as e:
      pass
